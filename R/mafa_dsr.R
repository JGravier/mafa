#' Distance to minimal bounding rectangle
#'
#' @description
...
#' the area of its minimal oriented bounding box (rectangle). DSR is normalized between `[0-1]`.
#'
#' @param sfpolygons sf object composed of polygons.
#' @return `sfpoygons` with DSR as a new column.
#'
...
mafa_dsr <- function(sfpolygons){
  ...
  listboundingbox <- lapply(listpolygons, mafa_mbboxoriented) # apply mafa_mbboxoriented() function
